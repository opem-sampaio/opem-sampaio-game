...
import connect.mta.matrp.core.DialogClientSettings;
import connect.mta.matrp.gui.ChooseServer;
import connect.mta.matrp.gui.HudManager;
import connect.mta.matrp.gui.Menu;
import connect.mta.matrp.gui.Menumap;
import connect.mta.matrp.gui.Notification;
import connect.mta.matrp.gui.Phone;
import connect.mta.matrp.gui.Login;
import connect.mta.matrp.gui.Speedometer;
import connect.mta.matrp.gui.chatedgar.ChatManager;
import connect.mta.matrp.gui.dialogs.Dialog;
import connect.mta.matrp.gui.keyboard.KeyBoard;
import connect.mta.matrp.gui.Passport;
...
public abstract class NvEventQueueActivity
    ...
    private Passport mPassport = null;

    private Menumap mMenuMap = null;
    ...
    protected boolean systemInit()
    {
        ...
        mPassport = new Passport(this);
        mMenuMap = new Menumap(this);
        //mInterfacesManager = new connect.mta.matrp.gui.keyboard.j5.a(this);
    ...
    public void callLauncherActivity()
    {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Intent launchIntent = getPackageManager().getLaunchIntentForPackage("connect.mta.launcher");
   ...
   // public void hideLogin() { runOnUiThread(() -> { mHudManager.HideLogin(); }); } //login
    
   // public void showLogin() { runOnUiThread(() -> { mHudManager.ShowLogin(); }); } //login
    ...
    public void showPhone() { runOnUiThread(() -> { mPhone.show(); } ); }

    public void hidePhone() { runOnUiThread(() -> { mPhone.close(); } ); }
    
    
    public void showLogin() { runOnUiThread(() -> { mLogin.show(); } ); }
    
    public void hideLogin() { runOnUiThread(() -> { mLogin.close(); } ); }

    public void showPassport(String nick,int pol, int vozrast) { runOnUiThread(() -> { mPassport.show(nick, pol, vozrast); } ); }

    public void showMenuMap() { runOnUiThread(() -> { mMenuMap.show(); } ); }

    public void hideMenuMap() { runOnUiThread(() -> { mMenuMap.hide(); } ); }
}
